...
describe("SongQueue", () => {
  ...
  describe("addSongsAt", () => {
    ...
    it("should throw error for invalid index", () => {
      ...
      assertThrows(() => queue.addSongsAt([createSong(2)], -1), Error, "Invalid index for queue insertion");
      assertThrows(() => queue.addSongsAt([createSong(2)], 2), Error, "Invalid index for queue insertion");
    });
  ...
  describe("skipSongs", () => {
    it("should throw error for non-positive skip count", () => {
      assertThrows(() => queue.skipSongs(0), Error, "Number of songs to skip must be positive");
      assertThrows(() => queue.skipSongs(-1), Error, "Number of songs to skip must be positive");
    });
  ...
  describe("shuffle", () => {
    ...
    it("should shuffle remaining songs", () => {
      ...
      const isDifferent = originalOrder.some((song: TestSong, i: number) => song.id !== newOrder[i].id);
