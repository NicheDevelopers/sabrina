  ...
  describe("resolve", () => {
    it("should resolve plain text as YouTube search query", () => {
      ...
      for (const query of testQueries) {
        ...
        expect(result.type).toBe(QueryType.YT_SEARCH);
    ...
    it("should resolve valid YouTube URLs as YouTube URL queries", () => {
      ...
      for (const url of testUrls) {
        ...
        expect(result.type).toBe(QueryType.YT_URL);
    ...
    it("should handle edge cases correctly", () => {
      ...
      expect(result1.type).toBe(QueryType.YT_SEARCH);
      expect(result1.payload).toBe(nonYoutubeUrl);
      ...
      expect(result2.type).toBe(QueryType.YT_SEARCH);
      expect(result2.payload).toBe(fakeYoutubeUrl);
    });
    ...
    it("should handle type checking correctly", () => {
      ...
      if (searchQuery.type === QueryType.YT_SEARCH) {
        ...
        expect(typeof searchQuery.payload).toBe("string");
      } else {
        // This should never happen
        fail("Search query was incorrectly typed");
      }
      ...
      if (urlQuery.type === QueryType.YT_URL) {
        // This should be accessible
        expect(typeof urlQuery.payload).toBe("string");
      } else {
        // This should never happen
        fail("URL query was incorrectly typed");
      }
    });
  });
});